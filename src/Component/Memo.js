...
export default function Memo(props) {
  ...
  let values ;
  let [ExchangeRateNew,setRate] = useState({})
  // const ExchangeRate = {
  //   USD: 1,
  //   AED: 3.6725,
  //   AFN: 84.8866,
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((response) => response.json())
      .then((data) => {
        let keyarray = Object.keys(data.conversion_rates);
  ...
  const handleConvert = useMemo(() => {
    ...
    const conversionValues =
      ExchangeRateNew[toCurrency] / ExchangeRateNew[fromCurrency];
